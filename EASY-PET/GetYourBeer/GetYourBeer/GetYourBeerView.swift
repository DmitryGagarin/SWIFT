...
struct GetYourBeerView: View {
    
    @State private var beers: [Beer] = []
    @State private var isNavigationBarHidden: Bool = false
    var beerShow = BeerShow()
    
    var body: some View {
        NavigationView {
            List(beers) { beer in
                VStack {
                    beerShow.pictures(for: beer)
                    beerShow.texts(for: beer)
                }
            }
...
class BeerShow {
    ...
    func pictures(for beer: Beer) -> some View {
        return AsyncImage(url: URL(string: beer.image)) { phase in
    ...
    func texts(for beer: Beer) -> some View {
        return HStack {
